...
- `schedulePeriodicWork` (`CANCEL_AND_REENQUEUE`) is for a user-chosen interval change. App start uses `ensurePeriodicWorkScheduled` (`KEEP`) — restarting the period on every launch would let a frequently-opened app never reach the interval. Default interval is 6h (`SettingsRepository.DEFAULT_AUTO_UPDATE_INTERVAL_HOURS`); `0` disables.
